	...
	constructor(props){
        ...
        this.state = {
     	...
     	document.addEventListener('click', e => {
			if (e.target.className === 'hamburger' || e.target.className === 'hamburger-box' || e.target.className === 'cart-indicator' || e.target.className === 'hamburger-inner' || e.target.className === 'cart-image' || e.target.className === 'cart-item' || e.target.className === 'cart-item-close' || e.target.className === 'button button-red menu-item-buy empty-button') {
				return;
			} else {
					...
					<div className={this.state.activeCart ? 'dropdown-cart' : 'dropdown-cart hide'}>
						...
						<div className="items">
							...
							<div className={!isEmpty ? 'hide' : 'display'}>
								...
								<a onClick={this.toggleCart} className="button button-red menu-item-buy empty-button">ВИБРАТИ ТОВАР</a>
							</div>
						</div>
						<div onClick={this.toCheckout} className={isEmpty ? 'hide' : 'button button-red menu-item-buy button-checkout'}>Оформити</div>
					</div>
					<img className="cart-image" onClick={this.toggleCart} src={CartImage} alt="" />
					<div className={isEmpty ? 'hide' : 'cart-indicator'} onClick={this.toggleCart}>{totalItems}</div>
				</div>	
			</div>
			<div className="pag">
				{renderPageNumbers}
