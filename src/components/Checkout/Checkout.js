...
function addError() {
	...
	let parentDiv = document.getElementsByClassName('client-head')[0];
...
class Checkout extends Component {
    ...
    render() {
    	...
    	if (localStorage.cart) {
   			local = JSON.parse(localStorage.cart);
			var cartItem = local.map(item => {
				return(
					<div className="_cart-item" key={item.id}>
			            ...
			            <span className="_cart-item-price">{item.price}</span>
		...
		return(
    		<div>
				...
				<div className="order">
					<h2 className="order-head">ОФОРМЛЕННЯ ЗАМОВЛЕННЯ</h2>
					<div className="order-block">
						<div className="order-block-head">Склад замовлення:</div>
						<a className="order-block-change">Редагувати замовлення</a>
						<div className="items">
							{cartItem}
						</div>
						<div className={this.props.isEmpty ? 'hide' : 'summ'}>Всього: <span>{totalAmount} грн</span></div>
					</div>
				</div>
				<div className="client">
					<div className="client-margin">
						<h2 className="client-head">ВАШІ ДАНІ</h2>
						...
						<div className={this.state.radio ? 'hide' : 'display'}>
							<label htmlFor="text" className="client-name"><span className="client-address">Адреса доставки</span></label>
						...
						<div className="button button-red client-button" onClick={this.order}>ВІДПРАВИТИ</div>
					</div>
				</div>
				<div className={this.props.confirm ? 'confirm' : 'hide'}>
					<div className="confirm-head">Твоє замовлення офрмлено!</div>
					{confirmText}
					<div className="button button-red menu-item-buy confirm-button" onClick={this.toPage}>Назад до головної</div>
				...
				<div className={this.props.isError ? 'confirm-error' : 'hide'}>
					...
					<div className="confirm-text">На сайті технічні проблеми. Для того, щоб здійснити замовлення - зателефонуйте за номером - +380 (98) 350 42 51, м. Львів, вул. Братiв Рогатинцiв, 21</div>
					<div className="button button-red menu-item-buy confirm-button" onClick={this.toPage}>Назад до головної</div>
