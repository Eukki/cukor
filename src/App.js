  ...
  constructor(props){
	...
	getProducts(parentId){
	  ...
	  // const url = "../src/products.json";
		...
		const url="https://cukor-lviv.herokuapp.com/menu/red";
		var config = {
	    		headers: {
	...
	order(name, tel, comment, isSelf, street, home, apartment) {
		...
		try {
			var order = JSON.parse(localStorage.cart);
		} catch(e) {
			console.log(e);
		}
		let orderList = [];
    	...
    	axios({
    		method: 'post',
    		url: 'https://cukor-lviv.herokuapp.com/order/red',
    		data: toReturn,
    		headers: { 
		...
		toReturn =
			<div>
			  <MenuPage 
				 ...
				 toBlackPage={this.props.toBlackPage}
